...
<header class="bg-primary shadow-lg sticky top-0 z-50 transition-all duration-300">
    <div class="container mx-auto px-4">
        <div class="flex justify-between items-center py-4">
            ...
            <div class="flex items-center space-x-2">
                <a href="./dashboard.php" class="flex items-center group">
                    ...
                    <span class="text-white font-bold text-xl group-hover:text-accent transition-colors duration-300">User Portal</span>
                </a>
            </div>
            ...
            <nav class="hidden md:flex items-center space-x-8">
                ...
                ?>
                <a href="../index.php" class="text-gray-300 hover:text-accent transition-all duration-200 flex items-center py-2 border-b-2 <?= $current_page === 'index.php' ? 'border-accent text-accent' : 'border-transparent' ?>">
                ...
                <a href="./manage_bookings.php" class="text-gray-300 hover:text-accent transition-all duration-200 flex items-center py-2 border-b-2 <?= $current_page === 'booking.php' ? 'border-accent text-accent' : 'border-transparent' ?>">
                    <i class="fas fa-tasks mr-2"></i> Bookings
                </a>
                <a href="./manage_services.php" class="text-gray-300 hover:text-accent transition-all duration-200 flex items-center py-2 border-b-2 <?= $current_page === 'manage_services.php' ? 'border-accent text-accent' : 'border-transparent' ?>">
                    <i class="fas fa-calendar-alt mr-2"></i> Service
                </a>
            </nav>
            ...
            <div class="relative" x-data="{ open: false }">
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-95" class="absolute right-0 mt-3 w-56 bg-white rounded-lg shadow-xl py-2 z-50 hidden" id="userDropdown">
                    ...
                    <a href="./profile_management.php" class="block px-4 py-3 text-sm text-gray-700 hover:bg-gray-100 transition-colors duration-200 flex items-center">
        ...
        <div id="mobileMenu" class="md:hidden hidden pb-4 pt-2 border-t border-gray-700 mt-2 space-y-2">
            <a href="../index.php" class="block py-2.5 px-3 text-gray-300 hover:text-accent hover:bg-primary-dark rounded-lg transition-colors duration-200 flex items-center <?= $current_page === 'index.php' ? 'bg-accent/10 text-accent' : '' ?>">
            ...
            <a href="./manage_bookings.php" class="block py-2.5 px-3 text-gray-300 hover:text-accent hover:bg-primary-dark rounded-lg transition-colors duration-200 flex items-center <?= $current_page === 'booking.php' ? 'bg-accent/10 text-accent' : '' ?>">
                <i class="fas fa-tasks mr-3 w-5 text-center"></i> Bookings
            </a>
            <a href="./manage_services.php" class="block py-2.5 px-3 text-gray-300 hover:text-accent hover:bg-primary-dark rounded-lg transition-colors duration-200 flex items-center <?= $current_page === 'manage_services.php' ? 'bg-accent/10 text-accent' : '' ?>">
                <i class="fas fa-calendar-alt mr-3 w-5 text-center"></i> Service
            </a>
            <a href="./profile_management.php" class="block py-2.5 px-3 text-gray-300 hover:text-accent hover:bg-primary-dark rounded-lg transition-colors duration-200 flex items-center <?= $current_page === 'profile_management.php' ? 'bg-accent/10 text-accent' : '' ?>">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Show dropdown on non-Alpine.js browsers
